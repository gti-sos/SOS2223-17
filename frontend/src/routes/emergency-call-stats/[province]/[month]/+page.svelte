<script>
  ...
  let updateTelefonica ='';
  let updateTelematica = '';
  let updateNumber = '';
  let updateYear = '';
  ...
  async function getCall() {
    ...
    updateTelematica = call.telematic_activation_organization;
    updateTelefonica = call.phone_call_activation_organization;
    updateNumber = call.emergency_call;
    updateYear = call.year;
  ...
  async function updateCall() {
    ...
    if (status == 200) {
      ...
      color = "danger";
      message =
        "Datos de entrada no válidos: compruebe las mayúsculas, tíldes y los campos númericos";
    } else if (status == 500) {
      flag = true;
      color = "danger";
  ...
  <Table hover>
    ...
    <tbody>
      <tr>
        ...
        <td
          ><Button color="primary" on:click={updateCall}>Actualizar</Button></td
        >
      </tr>
    </tbody>
  </Table>
